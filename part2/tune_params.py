...
try:
    if deneme:
        results_GridSearch.to_excel(f"tables-2/GridCV_{part}_{scale_samples}_{deneme}.xlsx", index= False, header= True, sheet_name="Search Results", float_format="%.6f")
    else:
        results_GridSearch.to_excel(f"tables-2/GridCV_{part}_{scale_samples}.xlsx", index= False, header= True, sheet_name="Search Results", float_format="%.6f")
