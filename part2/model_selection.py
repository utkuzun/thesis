...
hidden_layer_comp_df = take_hiddens_comp(activation=activation)
alpha_comp_df = take_alphas_comp(activation=activation)

hidden_layer_comp_df.to_excel(f"tables-2/metrics_eval_{part}_{scale_samples}.xlsx", index= False, header= True, sheet_name="hidden_layer", float_format="%.6f")
...
###############
# hidden layers
ax1.set_title("Hidden layers vs $R^{2}$ metric with no regularization")
ax1.set_ylabel("$r^{2}$")
...
# ########################
# Alphas
ax2.set_title(f"Alphas vs $R^{2}$ metric with (40,) hidden layers")
ax2.set_ylabel("$r^{2}$")
