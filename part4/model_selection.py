...
if deneme:
    results_GridSearch =  pd.DataFrame(pd.read_excel(f"tables-4/GridCV_{part}_{scale_samples}_{deneme}.xlsx", header=0, sheet_name="Search Results"))
else:
    results_GridSearch =  pd.DataFrame(pd.read_excel(f"tables-4/GridCV_{part}_{scale_samples}.xlsx", header=0, sheet_name="Search Results"))

def take_hiddens_comp(activation="tanh", base_alpha=0):
...
def take_alphas_comp(activation="tanh",base_hidden_layer="(40,)"):
...
hidden_layer_comp_df.to_excel(f"tables-4/metrics_eval_{part}_{scale_samples}_hidden.xlsx", index= False, header= True, sheet_name="hidden_layer", float_format="%.6f")
alpha_comp_df.to_excel(f"tables-4/metrics_eval_{part}_{scale_samples}_alphas.xlsx", index= False, header= True, sheet_name="alpha", float_format="%.6f")
...
if deneme:  
    plt.savefig(f"graphs-4/GridCV_{part}_{scale_samples}_{deneme}.png")
else:
    plt.savefig(f"graphs-4/GridCV_{part}_{scale_samples}.png")
